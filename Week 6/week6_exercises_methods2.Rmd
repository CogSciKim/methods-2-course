...
X <- matrix(cbind(rep(1,100),seq(1,100)),ncol = 2, nrow = 100)
Beta <- c(30,6)

df <- as.data.frame(cbind(X,X %*% Beta))

ggplot(df)+aes(x=V2,y=V3)+geom_point()+geom_smooth(method="lm")+theme_bw()
```
There are no errors

```{r}
df$errs <- rnorm(100,0,5)

df$yerrs <- df$V3+df$errs

ggplot(df)+aes(x=V2,y=yerrs)+geom_point()+geom_abline(slope=6,intercept=30)

```
Yes

```{r}
(solve(t(X)%*%X)%*%t(X))%*%matrix(df$yerrs)

lm(yerrs ~ V2, df)

```
The estimated parameters almost estimate the population parameters, however errors cause a distortion.
The coefficients match. Singularity is an issue when $|XX^{-1}| = 0$.
...
```{r}
design <- matrix(c(rep(1,length(mtcars$mpg)),mtcars$hp,mtcars$wt,mtcars$am),ncol=4)


(solve(t(design)%*%design)%*%t(design))%*%matrix(mtcars$mpg)
